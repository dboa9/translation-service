...
class MBZUAIParisAtlasChatService:
    def __init__(self):
        self.api_url = "https://api-inference.huggingface.co/models/MBZUAI/ParisAtlas-Chat-9B"
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if isinstance(result, list) and len(result) > 0:
                    ...
                    return "[MBZUAIParisAtlasChat] Unexpected API response format"
            except requests.exceptions.RequestException as e:
                logger.warning(f"API request failed (attempt {attempt + 1}/{self.max_retries}): {str(e)}")
                if attempt < self.max_retries - 1:
                    time.sleep(self.retry_delay)
                else:
                    return f"[MBZUAIParisAtlasChat] API request failed after {self.max_retries} attempts: {str(e)}"
            except Exception as e:
                logger.error(f"Translation error: {str(e)}")
                return f"[MBZUAIParisAtlasChat] Translation error: {str(e)}"
        
        return "[MBZUAIParisAtlasChat] Translation failed after all retries"

# Example usage
if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    service = MBZUAIParisAtlasChatService()
