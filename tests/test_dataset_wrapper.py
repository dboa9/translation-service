...
class TestDatasetWrapper:
    ...
    def setup(self, project_root_path, dataset_wrapper, credentials):
        ...
        logger.debug(f"TestDatasetWrapper set up with base_dir: {self.base_dir}, cache_dir: {self.cache_dir}")
    ...
    def _assert_columns_present(self, column_names: List[str], dataset_name: str, subset: str):
        ...
        for col in expected_columns:
            ...
            if col in aliases:
                assert any(alias in column_names for alias in aliases[col]), \
                    f"Neither '{col}' nor its aliases {aliases[col]} found in dataset for subset {subset}. Available columns: {column_names}"
            else:
