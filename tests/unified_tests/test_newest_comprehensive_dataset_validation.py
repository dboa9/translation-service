...
class TestNewestComprehensiveDatasetValidation(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    @staticmethod
    def load_config():
        config_path = Path("/mnt/c/Users/mrdbo/Documents/ReactDev/AWS/EC2/daija_dataset_tests_project/config/newest_dataset_loader_debug.yaml")
    ...
    def _validate_and_check_integration(self, dataset: Union[Dataset, DatasetDict, IterableDataset, IterableDatasetDict], dataset_name: str, subset: str, loader_type: str):
        # Validate dataset
        try:
            logger.info(f"Configuration for dataset: {dataset_name}, subset: {subset} - {self.config['debug']['datasets'][dataset_name]}")
